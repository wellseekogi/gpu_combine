...
async function handler(request,ip){
 ...
 if(route==="/config.js")return new Response("window.__RELAY_SELFHOST__=true;",{headers:{"Content-Type":"application/javascript"}});
...
server.listen(Number(process.env.RELAY_PORT??8788),process.env.RELAY_HOST??"127.0.0.1",()=>console.log("Relay ready at http://"+(process.env.RELAY_HOST??"127.0.0.1")+":"+(process.env.RELAY_PORT??8788)));
function stop(){server.close(()=>{db.close();process.exit(0)});}process.on("SIGINT",stop);process.on("SIGTERM",stop);
